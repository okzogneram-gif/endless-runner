(() => {
  ...
  const UNICORN_COIN_THRESHOLD = 100;
  const MYSTICAL_DISTANCE      = 500;
  ...
  function update(dt) {
    ...
    // ★ ユニコーンオーラアニメ更新
    if (isUnicorn) unicornAuraPhase += dt * 3;

    let scrollSpeed = speed;
    let distGain    = (speed * dt) / 50;
    ...
    // ★ ユニコーン進化判定（コイン100枚）
    if (!isUnicorn && coinsCollected >= UNICORN_COIN_THRESHOLD) {
      isUnicorn = true;
    ...
    if (boostRemaining > 0) { /* skip collision */ } else {
      for (const o of obstacles) {
        ...
        if (collidesBox(o)) {
          if (fireRemaining > 0) {
            ...
            for (let i = 0; i < 28; i++) {
              spawnParticle(cx + randRange(-6,6), cy + randRange(-8,8), `hsla(${Math.random()*40}, 100%, 60%, 0.95)`);
            }
  ...
  function drawHorse() {
    ...
    const COAT      = '#6b3a1d';
    const COAT_MID  = '#4d2711';
    const COAT_DARK = '#301708';
    const OUTLINE   = '#1a0904';
    const MANE      = '#1c0d05';
    const HOOF      = '#141414';
    const HOOF_HL   = '#3a2a1a';
    const SKIN      = '#f2c9a3';
    ...
    if (isUnicorn) {
      ...
      ctx.lineWidth = 1;
      ctx.beginPath();
      ctx.moveTo(hornX + 2,  hornY - 20); // 先端
  ...
  function drawUnicornHUD() {
    ...
    ctx.textAlign = 'right';
    ctx.textBaseline = 'top';
    ctx.fillText('🦄 UNICORN MODE', W - 12, mysticalBg ? 72 : 48);
    ctx.restore();
  }
  ...
  function render() {
    ...
    ctx.restore();
    drawStarFlash();
    drawUnicornEdgeGlow();   // ★ ユニコーンオーラ（画面端）
    drawBoostEdgeGlow();
    drawFireEdgeGlow();
    drawFireCountdown();
    drawMysticalHUD();       // ★ 神秘的背景HUD
    drawUnicornHUD();        // ★ ユニコーンHUD
  }
  ...
  window.__game = {
    ...
    forceGroundBox:      (x) => spawnGroundBox(x),
    forceCoinAt:         (x, y) => coins.push({ x, y, r: 10, collected: false, type: 'coin' }),
    // ★ デバッグ用
    getIsUnicorn:        () => isUnicorn,
    getMysticalBg:       () => mysticalBg,
    forceUnicorn:        () => { if (!isUnicorn) { isUnicorn = true; showUnicornEvolution(); } },
    ...
    UNICORN_COIN_THRESHOLD, MYSTICAL_DISTANCE,
  };
  ...
  charCards.forEach(card => {
    card.addEventListener('click', () => {
      ...
      selectedCharacter = CHARACTERS[parseInt(card.dataset.id)];
    });
  });
